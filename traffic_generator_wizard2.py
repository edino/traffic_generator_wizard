...
def generate_traffic(destination, count, size, interval, port, port_type, verbose=False, execution_time=None):
    ...
    if count == '' or count == float('inf'):
        count = sys.maxsize  # Set count to sys.maxsize if set to infinity or an empty string

    count = int(count)  # Convert count to int

    if execution_time:
        end_time = datetime.now(timezone.utc) + timedelta(seconds=execution_time)
    else:
        end_time = None

    while not end_time or datetime.now(timezone.utc) < end_time:
        if port_type.lower() == "both":
            tcp_count = count // 2
            udp_count = count - tcp_count
...
def main():
    ...
    if not count:
        ...
        if ask_execution_time:
            execution_time = int(get_user_input("\nEnter the number of seconds for script execution: "))
